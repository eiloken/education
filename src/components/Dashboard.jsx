...
function StorageRecalcPanel() {
    ...
    const handleRecalc = async () => {
        if (!window.confirm(
            'Recalculate storage sizes for all videos?\n\n' +
            'This scans every raw file and HLS folder on disk. ' +
            'It may take a moment for large libraries.'
        )) return;
        ...
        setError(null);
        try {
            const data = await videoAPI.recalculateStorage();
            setResult(data);
        } catch (e) {
    ...
    return (
        <Panel>
            <div className="flex items-center justify-between mb-4">
                <SectionTitle icon={HardDrive} title="Storage Recalculation" />
            </div>

            <p className="text-slate-500 text-xs mb-4 leading-relaxed">
                Storage sizes stored in the database only reflect the raw video file at upload time.
                Use this to rescan all videos and include HLS transcoded segment sizes in the totals.
                The Dashboard "Storage" stat card will update on next refresh.
            </p>
            ...
            {result && (
                <div className="mb-4 grid grid-cols-2 sm:grid-cols-4 gap-2">
                    <div className="bg-slate-800 rounded-lg p-2.5 text-center">
                        ...
                        <p className="text-slate-500 text-[10px] mt-0.5">Videos scanned</p>
                    </div>
                    <div className="bg-slate-800 rounded-lg p-2.5 text-center">
                        <p className="text-green-400 font-bold text-sm">{result.updated}</p>
                        <p className="text-slate-500 text-[10px] mt-0.5">Updated</p>
                    </div>
                    <div className="bg-slate-800 rounded-lg p-2.5 text-center">
                        <p className="text-blue-400 font-bold text-sm">{formatFileSize(result.totalRaw)}</p>
                        <p className="text-slate-500 text-[10px] mt-0.5">Raw files</p>
                    </div>
                    <div className="bg-slate-800 rounded-lg p-2.5 text-center">
                        <p className="text-amber-400 font-bold text-sm">{formatFileSize(result.totalHls)}</p>
                        <p className="text-slate-500 text-[10px] mt-0.5">HLS segments</p>
                    </div>

                    <div className="col-span-2 sm:col-span-4 bg-green-500/10 border border-green-500/20 rounded-lg p-2.5 flex items-center gap-2">
                        <CheckCircle2 className="w-4 h-4 text-green-400 shrink-0" />
                        <div className="text-xs text-green-300">
                            Total on-disk storage: <span className="font-bold">{formatFileSize(result.totalStorage)}</span>
                            {result.totalHls > 0 && (
                                <span className="text-green-400/60 ml-2">
                                    (including {formatFileSize(result.totalHls)} HLS)
                                </span>
                            )}
                        </div>
                    </div>
            ...
            >
                ...
                {loading ? 'Scanning…' : result ? 'Recalculate Again' : 'Recalculate Storage'}
            </button>
        </Panel>
    );
}
